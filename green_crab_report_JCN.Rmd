...
```{r, echo=FALSE} 
#hides code in the rendered version
mean(c(2,3,4))
...
```{r, results='hide'}
#hides output in the rendered version
mean(c(2,3,4))
message("this is a message")
```
Can click "insert" to put in the ticks for code!
...
mean(c(2,3,4))
message("this is a message")
#however, code is showing so you'll see the code to create the message
```




Alternatively, you can render r 'inline' using back-ticks and the contents need to begin with 'r '. For example the mean(c(2,3,4)) is `mean(c(2,3,4)` (that 3 was interpreted from the r code when we rendered the document!)
...
crabssf <- crabs %>% 
  ...
  st_transform(maritimes_eqdc_proj) # transform back to eqdc

map +
  geom_sf(data=crabssf) 

ggplot(crabssf)+
  geom_sf(data=NS, fill="burlywood", colour="burlywood", alpha=0.7) +
  geom_sf()
```

Hosted at https://barefooted23.github.io/SWC_example/green_crab_report_JCN.html